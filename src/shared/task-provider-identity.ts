...
export function normalizeTaskProviderIdentity(
  ...
  switch (provider) {
    ...
    case 'yunxiao':
      return {
        provider,
        accountId: normalizeNonEmptyString(raw.accountId),
        organizationId: normalizeNonEmptyString(raw.organizationId),
        organizationName: normalizeNonEmptyString(raw.organizationName),
...
export function isStoredTaskProviderIdentity(provider: TaskProvider, identity: unknown): boolean {
  ...
  switch (provider) {
    ...
    case 'yunxiao':
      return ['accountId', 'organizationId', 'organizationName', 'spaceId'].every((key) =>
        isNullableOptionalString(raw[key])
      )
  }
}
...
const TASK_PROVIDER_IDENTITY_FIELDS: Record<TaskProvider, readonly string[]> = {
  ...
  yunxiao: ['accountId', 'organizationId', 'organizationName', 'spaceId']
}
